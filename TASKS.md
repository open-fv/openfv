...
**Milestone M0:** `openfv run benchmarks/fifo/fifo.f --top fifo --parse-only` parses + elaborates and exits 0; CI green in all repos. **✅ ACHIEVED 2026-06-13** — all 7 P0.x cards merged; all 9 benchmarks exit 0 via the flagship CLI + pinned CIRCT; CI green across all 8 repos.
...
**Status:** ✅ flagship: `versions.txt` (CIRCT/LLVM/slang pins, provenance-documented), `scripts/bootstrap-circt.sh` (build CIRCT once + ccache + pin-stamp), `cmake/OpenfvVersions.cmake` + `cmake/OpenfvCIRCT.cmake` (single-sourced pins, ccache wiring, pin-match verification), `docs/BUILDING.md` (from-scratch Ubuntu LTS recipe). Per-repo CMake + hello-world `*-opt` copied into rtl-lowering/sva-frontend/btor2-emit when P0.1 landed (`build-templates/` staging removed from the flagship). **Fully verified 2026-06-12 on a dev box (WSL2, Ubuntu 24.04):** `bootstrap-circt.sh` built+installed the pinned CIRCT (pin stamp matches `versions.txt`); all three repos configure+build against it; all three `*-opt` tools pass `--help` + HW-IR round-trip; `circt-verilog` parses+elaborates SV→Moore. Acceptance found+fixed: `OpenfvVersions.cmake` needed `ENCODING UTF-8` (multibyte chars in versions.txt comments split lines), `BUILDING.md` had the wrong tool path, and a memory-constrained-host troubleshooting row was earned the hard way.
...
**Status:** ✅ Merged in all 8 repos (openfv#6 + p0.4-ci PR #1 in each sub-repo; the 3 CIRCT-repo build jobs went green after openfv#4's `ENCODING UTF-8` fix landed on `main` and were re-run). **Cache strategy:** the pinned CIRCT install is published as a GitHub Release asset on open-fv/openfv (tag `deps/circt-e003d2c8`, ~1.2 GB zstd → 5.2 GB, pin-stamped, reproducible from `versions.txt` via `scripts/bootstrap-circt.sh`); the three CIRCT repos' PR CI downloads it, verifies the pin stamp against `versions.txt`, then builds the `*-opt` tool and runs the hello-world smoke test (`--help` exits 0; `hw.module @top()` round-trips) — a cold CIRCT build never runs on a PR. Flagship CI: spec validation (`docs/specs/check_specs.py`, P0.6), submodule-consistency sanity, and an integration-test placeholder (real cross-repo test is P1.9); plus a nightly-regression skeleton (`nightly.yml`, P3.6 fills it) and a `workflow_dispatch` `seed-circt-cache.yml` that rebuilds + republishes the deps asset on a pin bump (cache strategy (a), for future bumps). Minimal repos (fv-engine, witness-remap, fv-benchmarks, fv-debug): DCO + scaffolding-sanity only (no code yet). Every repo also gets a clean-room DCO sign-off check. **Dependency on P0.3 PR openfv#4:** the three CIRCT repos' CI checks out flagship `main` for `versions.txt` + `cmake/`; their build job goes green only once openfv#4 lands the `OpenfvVersions.cmake` `ENCODING UTF-8` fix on `main` (without it CMake splits the em-dash comment line and configure fails — verified locally that with the fix the full configure+build+smoke passes against the relocated release asset). Warm-path PR build is well under the 15-min budget (per-repo build+smoke is seconds once the asset is cached).
...
**Status:** ✅ Merged (fv-benchmarks#1 designs + openfv#3 pin). 9 designs: fifo, counter, arbiter, ram_1p, ram_2p, regfile, shift_reg, stack, decoder, each with holding + violated SVA properties and `EXPECTED.md`. **`[FABLE]` resolution of P0.7's escalation E1:** the original designs used `default clocking`/`default disable iff`, which the pinned `circt-verilog` refuses (`ClockingBlock` unsupported); rewritten to explicit per-assertion `@(posedge clk) disable iff (!rst_n)` — semantics-preserving by IEEE 1800-2017 §14.11/§16.15. `stack.sv` also had an illegal part-select on a parenthesized expression (fixed with a named `top_idx` wire). Flagship submodule pin bumped to fv-benchmarks `main` so M0 runs the fixed designs.
...
**Status:** ✅ Merged (openfv#5). `bin/openfv` + `openfv_cli/` (Python stdlib only); all 10 CLI tests pass; exit codes per `docs/specs/cli.md` (0/10/11/13). Escalation E1 (benchmarks used `default clocking`, unsupported by pinned `circt-verilog`) resolved by the `[FABLE]` benchmark rewrite under P0.5, not by changing the CLI — the CLI was always correct. **M0 milestone verified green:** all 9 P0.5 designs pass `openfv run <design> --top <top> --parse-only` (exit 0) against the pinned CIRCT install. E2 (forwarding `.f` `+define+`/`-I` options to circt-verilog) deferred to M1 — cli.md doesn't yet spec the mapping; P1.x picks it up.
